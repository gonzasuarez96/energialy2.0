...
io.on("connection", (socket) => {
  ...
  socket.on("authenticate", (data) => {
    ...
    if (companyId) {
      userSockets[companyId] = socket.id;
      socketToCompanyId[socket.id] = companyId;
      console.log(`User ${companyId} connected with socket ID: ${socket.id}`);
      console.log(userSockets);
    } else {
      console.log("Authentication failed");
  ...
  socket.on("disconnect", () => {
    console.log("User disconnected socket:" + socket.id);
    const companyId = socketToCompanyId[socket.id];
    if (companyId) {
      delete userSockets[companyId];
      delete socketToCompanyId[socket.id];
      console.log(`User ${companyId} disconnected and removed from userSockets`);
      console.log(userSockets);
    }
  });
});
